...
Sprint 16 Completed — Multi-AI Subscription Coding Control Center / Task Router: turns NEMI into a control center for the user's own existing ChatGPT/Codex, Claude Code, and Gemini CLI subscriptions, dispatched alongside (never replacing) the existing 7-provider API system — a full architectural audit ran first (three parallel research passes over Agent Orchestration, Provider Management, and the Live Dashboard) confirming no existing extension point for CLI execution and no field genuinely missing from the dashboard except "Running Tests" and per-task execution mode; new `app/ai/cli_tools.py` detects each tool the same two-axis way Sprint 15.5's Ollama detection already did (`shutil.which` for installed, a credential-file-existence heuristic for authenticated — never a stored credential, never fabricated availability); a new pure `resolve_execution()` Task Router function (Auto mode, explicit-tool pinning, subscription-first policy) is exhaustively unit tested with no I/O; `agent_tasks` gained four additive columns (`execution_backend`/`cli_tool_id`/`files_changed`/`cli_exit_code`) and a new singleton `execution_policy` table, while the three CLI tool ids simply became valid values in the already-free-text `agent_provider_defaults.provider` column (Sprint 15.5) rather than needing a new per-role-preference table; `manager.py`'s `_execute()` was split into `build_task_prompt()`/`finalize_task_result()` so a CLI-dispatched task gets identical memory/knowledge-graph grounding to an API one; Electron's new `cli-tools.ts` (mirroring `build-runner.ts`'s existing spawn/stream pattern) owns actually spawning the CLI process, a git checkpoint-commit-diff safety sequence (refuses dispatch into a non-git project, checkpoints dirty trees first, commits the CLI's own changes second) replacing human-gated Apply/Reject for this direct-write execution path, and a hard project-wide single-flight lock so only one CLI agent ever runs at once; Settings gained one new "AI Coding Control" tab and the Live Dashboard gained one new "AI Coding Control" section (both extending existing shells, not new dashboards) plus the two genuinely-missing Sprint Center fields; found and fixed one real pre-existing gap during implementation — `create_milestone_pipelines()` had no way to know a per-role provider override was actually a CLI tool id, which would have made `run_cycle()` crash with `UnknownProviderError` the first time a user set one via the new Settings tab; verified via 37 new backend tests (208 total passing), the full offline suite (tsc/eslint/prettier/build/ruff/mypy all clean), and genuine live verification against this machine's real, actually-installed Claude Code CLI (confirmed installed+authenticated with its real binary path and version, Codex/Gemini correctly confirmed not installed) through a full real create-pipeline → Task Router routing → claim-dispatch → record-result lifecycle over HTTP; the Electron-side GUI pass was not completed this session — a pre-existing installed Alpha build from an earlier session held the single-instance lock and could not be forcibly terminated (Windows "Access is denied" on the process tree), a genuine environment permission wall rather than a code defect, so that layer is verified by code review and by mirroring `build-runner.ts`'s already-proven pattern instead

---

# NEXT MILESTONE

Sprint 16.1 (or Sprint 17)

Live Electron GUI verification of the Task Router / AI Coding Control UI end-to-end (Settings tab, dashboard section, the four routing buttons, a real dispatched CLI coding run writing real files) — deferred this sprint by a stray pre-existing process holding the single-instance lock, not by any code issue; see docs/SPRINT_16_REPORT.md

Real dispatch verification for Codex CLI and Gemini CLI specifically (only Claude Code CLI is installed on this development machine) — the detection and routing logic is identical and tested for all three, but an actual end-to-end CLI coding run has only been exercised for Claude Code
